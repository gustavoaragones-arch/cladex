...
export default async function BlogsPage() {
  ...
  return (
    <Container
      title="Transaction Resources | Cladex"
      className="pb-20 relative"
      description="Guides, frameworks, and checklists for buyers and sellers navigating real estate transactions."
    >
      ...
      <div className="pb-4 pt-4 md:pb-20 md:pt-10 relative px-4">
        <div className="mx-auto max-w-2xl sm:text-center pb-10 ">
          ...
          <p className="mt-2 text-lg text-gray-600">
            Guides, frameworks, and checklists for buyers and sellers navigating real estate transactions.
          </p>
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 max-w-6xl mx-auto mb-20">
        {sortedBlogs.length === 0 ? (
          <p className="text-lg text-gray-600 col-span-full text-center py-12">Resources coming soon.</p>
...
function BlogCard({ title, description, image, link }: any) {
  const truncate = (text: string) =>
    text.length > 100 ? text.substring(0, 200) + "..." : text;
  return (
    <Link
      href={link}
